...
# Building and running an image for [Sweden] WITHOUT compose

-   Clone this repo
-   Make sure the latest `pbf` file for sweden exists under: `./openrouteservice/src/main/files/sweden-latest.osm.pbf`
    -   http://download.geofabrik.de/europe/sweden-latest.osm.pbf
    -   Or uncomment the first 2 lines in `run_app.sh` to get the same behaviour whenever using `run_app.sh` to build and run the image
-   Customize `./config/ors-config.json.json` to fit the desired api profiles (car, bikes ...) and other configurations
-   Execute `run_app.sh` to build a fresh image and run it with the desired arguements which can also be customized inside `run_app.sh`
